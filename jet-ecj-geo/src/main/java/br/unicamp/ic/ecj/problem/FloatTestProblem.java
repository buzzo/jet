...
/**
 * Class for problems with {@link Float}.
 */
public class FloatTestProblem extends Problem implements TestProblem {
    ...
    public FloatTestProblem(final InstrumentedInstance instance, final Method method, final List<Integer> target) {
    ...
    public void evaluate(final EvolutionState state, final GEOSubpopulation geopop) {
        ...
        for (int i = 0; i < candidates.size(); i++) {
            ...
            for (int j = 0; j < ind.length; j++) {
                if (!(ind[j] instanceof FloatVectorIndividual)) {
                    state.output.fatal("Individual must be of type [" + FloatVectorIndividual.class.getName() + "]. It is ["
                        + ind[j].getClass().getName() + "]", null);
                }
                final FloatVectorIndividual dvInd = (FloatVectorIndividual) ind[j];
            ...
            final double fit = FloatTestProblem.SIMILARITY.evaluate(this.targetPath, CoveredPath.createCoveredPath(list));
