...
/**
 * Class for problems with {@link Integer}.
 */
public class IntTestProblem extends Problem implements TestProblem {
    ...
    public IntTestProblem(final InstrumentedInstance instance, final Method method, final List<Integer> target) {
        ...
        if (injection) {
            ...
            for (int i = 0; i < indexes.length; i++) {
                values[i] = helper.getInjectionValue(indexes[i]);
            }
        }
    }
    ...
    public void evaluate(final EvolutionState state, final GEOSubpopulation geopop) {
        ...
        for (int i = 0; i < candidates.size(); i++) {
            ...
            for (int j = 0; j < ind.length; j++) {
                if (!(ind[j] instanceof IntegerVectorIndividual)) {
                    state.output.fatal("Individual must be of type [" + IntegerVectorIndividual.class.getName() + "]. It is ["
                        + ind[j].getClass().getName() + "]", null);
                }
                final IntegerVectorIndividual ivInd = (IntegerVectorIndividual) ind[j];
                params.add(ivInd.genome[0]);
            }
            ...
            if (this.injection) {
                for (int j = 0; j < indexes.length; j++) {
                    // index starts at 1
                    params.add(indexes[j] - 1, values[j]);
                }
            }
            ...
            final double fit = IntTestProblem.SIMILARITY.evaluate(this.targetPath, CoveredPath.createCoveredPath(list));
